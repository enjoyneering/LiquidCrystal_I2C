...
void LiquidCrystal_I2C::setCursor(uint8_t colum, uint8_t row)
{
  ...
  send(LCD_INSTRUCTION_WRITE, LCD_SET_DDRAM_ADDR | (row_address_offset[row] + colum), LCD_CMD_LENGTH_8BIT);
}
...
/*
    ...
    Soft reset and 4-bit interface initialization
*/
/**************************************************************************/
void LiquidCrystal_I2C::initialization(void)
{
  ...
  some Arduino can start and run the code at 2.4v, so we'll wait 0.5 seconds
  */
  delay(500);
...
/*
    ...
    Wire.endTransmission() returned codes:  
     0 - success
     1 - data too long to fit in transmit data16
     2 - received NACK on transmit of address
     3 - received NACK on transmit of data
